"""
Ranking RAG Pipeline - With RRF (Reciprocal Rank Fusion)
"""
import logging
...
def rrf_fusion(result_per_query, k=60):
    """Reciprocal Rank Fusion: Combine rankings from multiple input lists"""
    logging.info("Starting RRF fusion process...")
    scores = {}
    ...
    for query_idx, results in enumerate(result_per_query):
        ...
        for rank, chunk in enumerate(results):
            ...
            logging.debug(f"Rank {rank}: score = {scores[key]['score']:.4f}")
...
def chat_loop(openai_client, qdrant_client, collection_name):
    ...
    while True:
        ...
        # Apply RRF fusion
        # fused_chunks = rrf_fusion(all_chunks)

        # Apply RRF fusion with semantic scoring
        fused_chunks = rrf_fusion_with_semantic_score(all_chunks)
        
        # Sort and select top 5
        sorted_chunks = sorted(fused_chunks, key=itemgetter("score"), reverse=True)
...
def main():
    setup_logging()
    logging.info("Starting RAG pipeline with RRF ranking...")
