...
def rrf_fusion(result_per_query, k=60):
    ...
    for query_idx, results in enumerate(result_per_query):
        logging.info(f"Processing results for query {query_idx}")
        logging.info(f"RESULT: {results}")
        for rank, chunk in enumerate(results):
            logging.debug(f"  Rank: {rank}, Text length: {len(chunk['text'])}")
            key = chunk["text"]
            
            if key not in scores:
                logging.debug(f"New chunk added to scores")
                scores[key] = {
                    "text": chunk["text"],
                    "score": 0
                }
...
def chat_loop(openai_client, qdrant_client, collection_name):
    ...
    while True:
        ...
        logging.info(f"FUSED CHUNKS: {fused_chunks}")
        #sorting
        top_chunks = sorted(fused_chunks,key=lambda x: x["score"], reverse=True )[:5]

        unique_chunks = deduplicate_chunks(top_chunks)
